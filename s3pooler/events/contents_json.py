...
def common_json(json_model, user):
    return {'response_code': json_model.code()}

def comment_json(json_model, user):
     comment_json = common_json(json_model, user)
     comment_json['message'] = json_model.request('message')
     return comment_json

def user_json(json_model, user):
    user_json = common_json(json_model, user)
    keys = ['facebook_id','instagram_id','about_me','image_profile']
    user_json.update(_dict(json_model.response, keys))
    return user_json

def post_json(json_model, user):
    post_json = common_json(json_model, user)
    keys = ['post_type','title','text','tags','user_mark','images']
    post_json.update(_dict(json_model.request, keys))
    return post_json

def reaction_json(json_model, user):
    react_json = common_json(json_model, user)
    react_json['reaction_code'] =json_model.request('liked')
    return react_json

def scroll_json(json_model, user):
    scroll_json = common_json(json_model, user)
